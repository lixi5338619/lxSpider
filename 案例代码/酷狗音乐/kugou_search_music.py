...
def search(keyword):
    ...
    KEY_CODE = "NVPh5oo715z5DIWAeQlhMDsWXXQV4hwtbitrate=0clienttime={time}clientver=2000dfid=-inputtype=0iscorrection=1isfuzzy=0keyword={keyword}mid={time}page=1pagesize=30platform=WebFilterprivilege_filter=0srcappid=2919tag=emuserid=-1uuid={time}NVPh5oo715z5DIWAeQlhMDsWXXQV4hwt"
    p = KEY_CODE.format(time=millis, keyword=keyword)
    signature = getSignature(p)
    URL_SEARCH = "https://complexsearch.kugou.com/v2/search/song?keyword={keyword}&page=1&pagesize=30&bitrate=0&isfuzzy=0&tag=em&inputtype=0&platform=WebFilter&userid=-1&clientver=2000&iscorrection=1&privilege_filter=0&srcappid=2919&clienttime={time}&mid={time}&uuid={time}&dfid=-&signature={signature}"
    url = URL_SEARCH.format(keyword=keyword, time=millis, signature=signature)
    response = requests.get(url=url)
    json_dict = response.json()
    lists = json_dict.get("data", {}).get("lists")
    return lists
